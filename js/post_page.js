(async function(){
  ...
  try{
    ...
    if (switchLang) switchLang.href = cfg.LANG_SWITCH + encodeURIComponent(p.id);
    ...
    el.innerHTML = `
      <article class="post">
        ...
        ${(p.links&&p.links.length)?`<div class="post-links">${p.links.map(l=>`<a href="${l.url}" target="_blank" rel="noopener">${l.label||'Lien'}</a>`).join(' ')}</div>`:''}
        <p class="back"><a href="${document.documentElement.lang==='fr'?'./':'../'}">← ${document.documentElement.lang==='fr'?'Retour':'Back'}</a></p>
      </article>
    `;
  }catch(e){

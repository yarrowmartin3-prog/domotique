(async function () {
  ...
  function render(posts){
    ...
    filtered.sort((a,b)=>new Date(b.date||0)-new Date(a.date||0)).forEach(p=>{
      ...
      const img=node.querySelector('.thumb'), title=node.querySelector('.title'), meta=node.querySelector('.meta'),
            sum=node.querySelector('.summary'), links=node.querySelector('.links');
      img.src=p.image||''; img.alt=p.title||''; title.textContent=p.title||'';
      meta.textContent=[fmtDate(p.date||''),(p.tags||[]).join(' • ')].filter(Boolean).join(' — ');
      sum.textContent=p.summary||'';
      (p.links||[]).forEach(l=>{ const a=document.createElement('a'); a.href=l.url; a.textContent=l.label||'Link'; a.target="_blank"; a.rel="noopener"; links.appendChild(a); });
